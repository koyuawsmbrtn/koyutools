...
for surl in tracks:
    ...
    if not trackid in downloaded:
        ...
        if not os.path.exists(filename):
            ...
            print(title)
            query = artist + " - " + title + " Auto-generated by YouTube"
            print(query)
            cmd = ["yt-dlp", "--no-continue", "--add-metadata", "-x", "--prefer-ffmpeg", "--extract-audio", "-v", "--audio-format", "mp3", "--output", "audio.%(ext)s", "ytsearch:\"" + query + "\"", "--no-playlist"]
